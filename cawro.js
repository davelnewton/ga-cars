...
// TODO GENE Number of wheels
// TODO GENE Wheel friction
// TODO GENE Number of vertices?
// TODO Change gene expression, mutation, and crossover
// TODO Only mutable path
// TODO Hall of Champions
...
cw_Car.prototype.__constructor = function(car_def) {
  ...
  this.wheels = [];
  for (i = 0; i < car_def.wheelCount; i++) {
...
function cw_createChassisPart(body, vertex1, vertex2, density) {
  ...
  // TODO See identical code in `cw_createWheel`
  fix_def                   = new b2FixtureDef();
...
function cw_createWheel(radius, density) {
  ...
  // TODO See identical code in `cw_createChassisPart`
  var fix_def               = new b2FixtureDef();
  fix_def.shape             = new b2CircleShape(radius);
  fix_def.density           = density;
  fix_def.friction          = 1.5;
  fix_def.restitution       = 0.2;
  fix_def.filter.groupIndex = -1;
...
function cw_createRandomCar() {
  var i
    ...
    , usedWheelVertices = {}
    , car_def = {}
    , carWheels = App.rndInt(2, 4)
    ;

  car_def.wheelCount    = carWheels;
  car_def.wheel_radius  = [];
  car_def.wheel_density = [];
  car_def.wheel_vertex  = [];

  for (var i = 0; i < car_def.wheelCount; i++) {
    car_def.wheel_radius[i]  = App.rnd(wheelMinRadius, wheelMaxRadius);
    car_def.wheel_density[i] = App.rnd(wheelMinDensity, wheelMaxDensity);
  }

  car_def.chassis_density = App.rnd(chassisMinDensity, chassisMaxDensity);
...
/* Favor higher-scoring cars as parents */
function cw_getParents() {
  var parentIdx
    , weight      = 0.65
    , parentSplit = Math.floor(generationSize * weight)
    ;

  if (Math.random() <= weight) {
    parentIdx = App.rndInt(0, Math.floor(generationSize / 2));
  } else {
    parentIdx = App.rndInt(0, generationSize -1);
  }
...
function cw_makeChild(car_def1, car_def2) {
  ...
  for (i = 0; i < newCarDef.wheelCount; i++) {
    if (variateWheelParents) {
...
function cw_chooseParent(currParent, attributeIndex) {
  if ((swapPoint1 == attributeIndex) || (swapPoint2 == attributeIndex)) {
    return (currParent == 1) ? 0 : 1;
  }
...
function cw_kill() {
  var avgspeed = (myCar.maxPosition / myCar.frames) * box2dfps
    , position = myCar.maxPosition
    , score    = position + avgspeed
    ;
